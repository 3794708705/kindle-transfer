# KindleTransfer — Kindle 智能传书助手 V0.2.2
...
V0.2.2 是稳定版：修掉了会导致界面冻结的根因，并把日志噪音降到可用。

**已修复**

| 问题 | 原因 | 修复 |
|------|------|------|
| 界面冻结 / 程序无响应（AppHangB1） | 设备扫描跑在界面线程的 2 秒定时器上，一次卡住的 USB 调用就冻结整个窗口 | 扫描移到独立线程（`device_scanner.py`） |
| 日志淹没真实事件 | 每 2 秒写 3 行 `Found volume`，约 13 万行 / 天 | 仅在**卷集合变化时**记录，稳定状态完全静默 |
| 启动日志版本号错误 | 硬编码 `V0.1` | 统一为 `APP_VERSION` |

**实测**

- 30 秒空闲运行：日志新增 **11 行**（全部是启动信息），轮询只产生 **1 行**；修复前约 45 行
- 一次正常扫描耗时 **0.8 ms**（最慢 1.7 ms），线程化没有性能代价
- 回归测试：把扫描换成「永不返回」，断言界面定时器仍持续触发——此测试在修复前无法通过

**已知残留风险（未修，有意为之）**

「智能传送」前的二次校验（`validate_current_device()`）仍是**界面线程上的设备 I/O**，其中包含一次对 Kindle 的**写探针**。设备卡住时这一步同样会阻塞界面。

之所以没动：修它必须把传送入口改成异步，会重构传送主流程——而传送是本程序最核心的功能。为了一个「需要设备卡住 + 用户恰好在那一刻点击传送」的低概率场景去改动核心路径，风险大于收益。它每次传送只调用一次，不像轮询那样每 2 秒必然触发。

需要时可单独处理，改法：把校验放进一个短生命周期的校验线程，界面先显示「正在校验设备…」，校验通过后继续。
...
当前：**188 passed, 1 skipped, 0 failed**。
